...
buddhaposx, buddhaposy, buddhacount, buddhay, buddhax, imgx, imgy, xpostable, ypostable, x):
	resultre, resultim, resultresqr, resultimsqr = (0), (0), (0), (0)
	global result
	global counter
	global repeatcount
	...
	for a in range(0,round(iters[0])): #Loops for n amounts [iteration count]
		...
		if ship:
			...
			resultre = (resultresqr) - (resultimsqr) + (c.real[0])
			resultimsqr = (resultim) * (resultim)
		...
		if vlambda:
			resultim = (resultre) * (resultim) * (2.0) *(c.real + c.imag) + (c.imag)
			resultre = ((resultresqr) - (resultimsqr)) *(c.real + c.imag) + (c.real)
			resultimsqr = (resultim) * (resultim)
			resultresqr = (resultre) * (resultre)
		#if testfract:
...
im = image.new('RGB', (round(imagelength), round(imageheight)), color = "black") #creates imag[0]e
...
if buddha and nebula == False: #Reconstructs imag[0]e data from arrays used within imag[0]e
	...
	for a in range(0, round(height[0])):
		...
		imgx, imgy = 0, imgy + 1
if nebula: #Runs the above code three times with minor changes so that color imag[0]es can be created
